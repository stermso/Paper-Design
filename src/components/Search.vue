<template>
    <div>
        <p v-if="m!=''" class="booktitle">搜索结果:{{m | handletitle}}</p>
        <p v-if="m==''" class="booktitle">搜索结果:{{keyword}}</p>
<div class="bookcontent">
    <div v-for="item of result" :key="item._id">
        <div class="bookmsg">
            <img v-lazy="item.imgUrl" @click="handleToggle(item._id)" alt="">
    ...
    export default {
        ...
        methods:{
            ...
            handleCurrentChange(val){
                ...
                this.result = this.allresult.slice(com,val*12-1)
            },
            jumpChange(val){
                console.log(val)
        ...
        mounted(){
            var {m,keyword} = this.$route.query;
            this.keyword = keyword;
            if(m){
            this.$http.get(`/search?m=${m}&keyword=${keyword}`).then(res=>{

                    if(res.data.msg=="无结果"){
                    this.m = res.data.msg;
                }else{
                this.m = m;
                this.allresult = res.data.res
                this.result = res.data.res.slice(0,12)
                this.total = Number(res.data.res.length);
                }
            })
            }else{
                this.$http.get(`/search?keyword=${keyword}`).then(res=>{
                    console.log(res.data.res)
                    var arr = [];
                    let str = Object.values(res.data.res)
                    str.forEach(item=>{
                        item.forEach(item=>{
                            arr.push(item)
                        })
                    ...
                    console.log(arr)
                    this.allresult = arr;
                    this.result = arr.slice(0,12)
                    this.total = Number(arr.length)
                })
            }
        }
    }
